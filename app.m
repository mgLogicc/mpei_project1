...
% Check if bloom_filter does not exist or is empty
if ~exist('bloom_filter', 'var') || isempty(bloom_filter)
...
%% Naive-Bayes Classifier Training
    ...
    %    shingles = generateShingles(fakeTrainNews{i}, kshingles); 
    ...
    %    shingles = generateShingles(realTrainNews{i}, kshingles);
    %    signature = minhashSignatures(shingles, numHash, p, R); 
    %    
    ...
    if strcmp(class,'Fake')
        bloom_filter = insert(link_given, bloom_filter, k);
    end
    % Se a noticia for classificada como falsa
    % Adiciona a fonte passada ao bloom filters

    numTrain = length(trainNews);
    trainSignatures = [realSignatureMatrix, fakeSignatureMatrix];
    
    %for i = 1:numTrain
    %    shingles2 = generateShingles(trainNews{i}, kshingles);
    %    trainSignatures(:, i) = minhashSignatures(shingles2, numHash, p, R);
    %end
    
    shingles = generateShingles(new, kshingles);
    signature = minhashSignatures(shingles, numHash, p, R);

    similJacc = sum(trainSignatures == signature, 1) / numHash;
    
    load('Files/similJacc.mat')
    % Ordenar
    ...
    for i = 1:5
        if newIdx(i) < realTrainSize
            fprintf('Real new : %s\n', realTrainNews{newIdx(i)});
        else
            fprintf('Fake new : %s\n', fakeTrainNews{newIdx(i) - realTrainSize});
        end
